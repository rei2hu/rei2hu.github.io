...
const toMove = files.filter((file) => file.endsWith(".css") || file.endsWith(".ttf"));
...
fs.promises.mkdir("built/style", { recursive: true }).then(() => {
  toMove
    .forEach((file) => {
      ...
      fs.writeFileSync(path.resolve("built", "style", file), minified.warnings.length ? contents : minified.styles);
    });
});
