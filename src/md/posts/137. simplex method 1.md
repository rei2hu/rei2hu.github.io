...
This leaves us with the set of inequalities
...
![feasible region for hours only](/blobs/137/single.png)
...
(24, 0). If the implicit $x \geq 0$ and $y \geq 0$ constraints didn't exist,
then it could even make sense to go even further to something like (n, 24 - n)
where $n$ approaches infinity.

Now, going back to the original problem, with this new intuition, we now know
that the optimal solution should occur at the ends of the constraint lines,
barring the one special case. This is known as the
...
![feasible region with corners](/blobs/137/points.png)
